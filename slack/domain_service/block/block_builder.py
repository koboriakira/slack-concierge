...
class BlockBuilder:
    ...
    def add_datepicker(self, action_id: str, header: str, placeholder: str, initial_date: str | None = None) -> "BlockBuilder":
        accessory = {
            "type": "datepicker",
            "action_id": action_id,
    ...
    def add_timepicker(self, action_id: str, placeholder: str, initial_time: str | None = None, label: str | None = None) -> "BlockBuilder":
        element = {
            "type": "timepicker",
    ...
    # def add_blocks_for_append_comment(self, page_id: str, page_title: str) -> 'BlockBuilder':
    #     """ Notionのコメント追加のブロックを作成する """
    #     builder = self.add_section(text=page_title)
    #     return builder.add_action_button(
    #         action_type=InteractiveActionType.APPEND_COMMENT,
    #         title="コメントを追加",
    #         value=page_id
    #     )

    # def add_action_button(self, action_type: InteractiveActionType, title: str, value: str, style: str = "primary") -> 'BlockBuilder':
    #     element = {
    #         "type": "button",
    #         "text": {
    #             "type": "plain_text",
    ...
    #         action_blocks[0]["elements"].append(element)
    #         return BlockBuilder(blocks=blocks)
    #     action_block = {
    #         "type": "actions",
    #         "block_id": action_type.value,
    #         "elements": [
    #             {
    #                 "type": "button",
    #                 "text": {
    ...
    #         ]
    #     }
    #     return BlockBuilder(blocks=self.blocks + [action_block])
    ...
    def append_context(self, data: dict) -> "BlockBuilder":
        renew_blocks = []
        renew_context_data = {}

        # contextブロック以外のブロックを入れたり、元のcontextブロックのデータを取り出したりする
        for block in self.blocks:
            if block["type"] == "context":
                original_text_data = json.loads(block["elements"][0]["text"])
                renew_context_data.update(original_text_data)
            else:
                renew_blocks.append(block)

        # データを追加して新しいcontextブロックを作成する
        renew_context_data.update(data)
        renew_blocks.append({
            "type": "context",
            "elements": [
                {
                    "type": "plain_text",
                    "text": json.dumps(renew_context_data, ensure_ascii=False),
                },
            ],
        })
        return BlockBuilder(blocks=renew_blocks)
